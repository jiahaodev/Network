/****************************************************
    ...
    日期：2019/11/13 0:36:1
	功能：网络管理的核心类
    负责：网络连接、通信、断开、心跳检测
          接收消息的队列处理
          事件、消息  的 添加/移除注册， 消息分发机制
          对外提供Update接口，用于不断触发消息分发与心跳检测
*****************************************************/
...
public class NetManager : MonoBehaviour
{
    ...
    #region 事件监听、移除，分发（事件主要用于处理socket连接、关闭等）
    //事件枚举
    ...
    #region 消息监听、移除，分发（消息主要用于处理游戏内的具体通信）
    //消息委托类型
    ...
    public static void Connect(string ip, int port)
    {
        ...
        {
            Debug.Log("Connect fail,alread connected!");
            return;
        }
        if (isConnecting)
        {
    ...
    public static void Close()
    {
        ...
        {
            isClosing = true; //如果数据未发送完全，标记。在发送回调中关闭
        }
        else
        {
            socket.Close();
